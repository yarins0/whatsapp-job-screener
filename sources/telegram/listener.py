...
def _build_text(message) -> str | None:
    """Return the message text with any Telegram link preview appended.

    Uses Telegram's own MessageMediaWebPage data (zero latency, no HTTP call).
    URL-based enrichment for empty previews is handled centrally by the ingest
    API so that WhatsApp and Telegram messages receive identical treatment.
